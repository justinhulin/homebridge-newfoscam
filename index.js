...
const StreamingDelegate = require("homebridge-camera-ffmpeg/dist/streamingDelegate");
...
module.exports = function (homebridge) {
  ...
  homebridge.registerPlatform("homebridge-newfoscam", "NewFoscam", FoscamPlatform, true);
}

function FoscamPlatform(log, config, api) {
  this.log = log;
  this.config = config || {"platform": "NewFoscam"};
...
FoscamPlatform.prototype.configureCamera = function (mac) {
  ...
  let cameraSource = new StreamingDelegate(hap, thisCamera, self.log, videoProcessor);
  ...
  self.api.publishCameraAccessories("NewFoscam", [newAccessory]);
